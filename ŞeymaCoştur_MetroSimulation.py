...
class MetroAgi:
    ...
    def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[Istasyon], int]]:
        ...
        ziyaret_edildi = set()
        # Öncelik kuyruğu: (toplam_maliyet, benzersiz_id, mevcut_istasyon, rota, toplam_sure)
        ...
        while pq:
            ...
            for komsu_istasyon, sure in mevcut_istasyon.komsular:
                if komsu_istasyon not in ziyaret_edildi:
                    # Yeni rotayı ve süreyi hesapla
                    yeni_rota = mevcut_rota + [komsu_istasyon]
                    yeni_sure = mevcut_sure + sure
                    
                    # Heuristic: Tahmin edilen kalan süre (basit olarak 0 aldık)
                    tahmin_maliyet = yeni_sure
                    
                    # Öncelik kuyruğuna ekle
                    heapq.heappush(pq, (tahmin_maliyet, id(komsu_istasyon), komsu_istasyon, yeni_rota, yeni_sure))
